...
public:
	...
	void QueryByPairs_Baseline(int k, int r, const vector<Edge>& pairs, const vector<Index>& weightIndex, ResultMap& results);
	void QueryByPairs_Maintain(int k, int r, DynamicClique* dynamicClique, const vector<Edge>& pairs, const vector<Index>& weightIndex, ResultMap& results, int method);
	...
	ResultMap BottomUpQuery_Baseline(int k, int r, vector<int> queryAttrs); // Basic
	ResultMap BottomUpQuery_KRCore(int k, int r, vector<int> queryAttrs); // Basic+KRCore
	ResultMap BottomUpQuery_Maintain(int k, int r, vector<int> queryAttrs, int method); // Basic+IEMC (the parameter "method" is for specifying the IEMC method(MCMEI/IMCE/NIEMC))
	ResultMap BottomUpQuery_Pruning(int k, int r, vector<int> queryAttrs); // Incremental
	ResultMap BottomUpQuery_Pruning1(int k, int r, vector<int> queryAttrs, int method); // Incremental, using different methods for IEMC (the parameter "method" is for specifying the IEMC method(MCMEI/IMCE/NIEMC))
